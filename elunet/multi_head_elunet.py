...
class ELUnet(nn.Module):
    ...
    def forward(self,x):
        ...
        logits = self.out_conv(x_dec_1)

        # ---- Classification Mask probablities
        avg_pool = self.adaptiveAvgPool(x_enc_4).view(-1,256)
        class_probablities = self.classifier(avg_pool)

        return torch.sigmoid(logits),class_probablities